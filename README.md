...
**После дорешивания:** Решал долго, постоянно были какие-то трудноуловимые ошибки. Записывать значения айдишников на каждом шаге не получилось, вылетело по
памяти. Вместо этого храню в хешмапе массивы истории изменения каждого айдишника, с шагами, и каждый раз ищу изменение на шаге предшествующем требуемому.
([код](main_round/3/3.go), полностью решена)
...
**После дорешивания:** Простая задача, решил быстро. Есть последовательность *..., a, b, ..., c, d, ...* . Нужно найти количество пар, у которых *b+c=a+d*. Это выражение можно сразу
заменить на эквивалентное *b-a=d-c*, разделив числа на левые и правые. Даже простой перебор (как в сортировке пузырьком) даёт частичное решение, и 11 баллов.
Осталось только немного ускорить — количество левых разностей пишем в хэшмапу, а для каждой правой ищем количество существующих левых. И добавляем его
к общему количеству. ([код](main_round/6/6.go), полностью решена)


**7 [Крестики-нолики (Middle)](main_round/7/)**, 35 баллов  
**После дорешивания:** Долго муторно решал чудовищным тракторизмом, наплодив 380 строк кода. Сначала для каждых строки/столбца/диагонали смотрю наличие
существующих выигрышных ситуаций. Одновременно записываю в хэшмапу количество крестиков в этих строке/столбце/диагонали, чтобы потом их можно было пропускать.
Потом перебираю каждый элемент, если в него можно поставить крестик, и он стоит на перспективной строке/столбце/диагонали — то ползу считать сумму 
вверх-вниз, влево-вправо, по диагонали. ([код](main_round/7/7.go), полностью решена)

Кстати тестовые данные они отдают не все — в архиве нету третьего теста, а он у меня отлетал. И ничего похожего на него нету. И вывод ошибки в этом случае
без конкретики.

**8 [Галерея искусств (Middle)](main_round/8/)**, 40 баллов  
**После дорешивания:** Сначала сделал двумерную таблицу флагов, где строки — номера картин, столбцы — номера коробок, а значение — влезает ли одно в другое.
Ищешь коробку, в которую в текущем моменте влезает наибольшее количество картин (сумма элементов столбца), удаляешь все строки с картинами, и столбец с
коробкой (в идеале, я просто забил). Это решение частичное, отлетает по скорости исполнения, и даёт 6 баллов. Чуть ускоряем его добавив хэшмапу, с предыдущим 
количеством картин в коробке (для пропуска циклов), оно уже дотягивает до 20 баллов. Его я успел проверить и  [выложил](main_round/8/8_partial.go).

Но полное решение требует другого подхода... Во-первых, у коробки есть длинная сторона, и короткая, этот момент выравниваем, длина всегда больше ширины.
А во-вторых, коробка с меньшей длиной нужна только в одном случае — если у неё ширина больше чем у длинной. Если мы отсортируем (не вложенные) коробки по возрастанию длины, то ширина будет уменьшаться. Поэтому сначала мы сортируем все коробки по возрастанию длины, с вложенной сортировкой по возрастанию ширины.
Тогда самая ценная коробка, с наибольшей длиной и шириной окажется справа. Потом бинарным поиском (есть в пакете sort) мы быстро найдём минимальный индекс,
чтобы влезть по длине. Теперь нам нужно найти максимальный индекс, чтобы влезть по ширине. Для этого я создал отдельный массив, где хранятся максимальные
с правого конца массива значения ширины. Он нам даёт максимальное значение индекса. А вот как оперировать этими максимальными и минимальными индексами
я проверить не успел — время вышло. По изначальной логике нужно использовать максимальный, но он дал неправильные ответы на 18-м тесте. Если брать
минимальный, то он дал нормальное решение на 18-м тесте, но целиком верность решения я уже не проверил. Скорее всего придётся искать уже имеющиеся
значения в диапазоне. И скорее всего придётся добавить сортировку картин, чтобы они приходили в нужном порядке. Это непроверенное решение я тоже
[выложил](main_round/8/8_try.go).
...
## Выводы
 ...
 - [Ошибочный вывод] ~~Задачи матанистые, учась на инженера я с таблицей умножения остатков не сталкивался. Можно нанять в день контеста репетитора по олимпиадным задачам. Или среди родственников его найти.~~
 ...
 - [Никому нахер не сдалась] ~~Сейчас все общаются в телеге. Я под эту тему создал группу <https://t.me/route256_sliv>, туда можно скинуть способ решения если решил какую-то задачу. Ну или на других конкурсах ищи группы по теме или создавай свою.~~
 ...
 - Нюансы Go жрут очень много времени. *Scanln()* тут работает с своими причудами. Всякие многомерные карты и срезы с непривычки выносят. Или какой-нибудь форматированный вывод.
 - Непредсказуемая сложность решения компенсируется возможностью посмотреть, что решили топовые участники. Они не сильные, они взялись за нужную задачу.
